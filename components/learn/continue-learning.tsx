...
export function ContinueLearning() {
  const active = tracks.filter((t) => t.progress > 0).sort((a, b) => b.progress - a.progress);
        ...
        <div className="grid md:grid-cols-2 gap-px bg-foreground/10 border border-foreground/10">
          {active.map((track) => (
            <Link
              ...
              className="group bg-background p-6 lg:p-8 flex items-center gap-6 hover:bg-foreground/[0.02] transition-colors"
            >
              <div className="shrink-0 w-14 h-14 border border-foreground/10 flex items-center justify-center font-display text-xl">
                {track.number}
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-4 mb-2">
                  <h3 className="text-lg font-medium truncate group-hover:translate-x-1 transition-transform">
                    {track.title}
                  </h3>
