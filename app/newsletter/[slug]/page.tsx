...
export default async function IssuePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <main className="relative min-h-screen overflow-x-hidden noise-overlay">
      <SiteNav />
      <article className="mx-auto max-w-[820px] px-6 pt-36 pb-20 lg:pt-44">
        ...
        <div className="mb-6 flex items-center gap-3 font-mono text-xs uppercase tracking-widest text-muted-foreground">
          <Newspaper className="h-4 w-4" aria-hidden="true" />
          Issue {issue.number} / {issue.topic} / {issue.date}
        </div>
          ...
          {issue.sections.map((section, index) => (
            <section key={section} className="border-t border-foreground/10 pt-8">
              ...
              <p className="mt-4 leading-8 text-muted-foreground">
                AIByDM tracks this topic through the lens of what learners can build, test, and ship.
                Use this issue as a signal layer, then jump into the related platform areas to keep going.
              </p>
            </section>
          ))}
        </div>
